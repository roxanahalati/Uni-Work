...
class Drone():
    ...
    def checkVisited(self,vertex):
        if vertex in self.stack:
            return True
        return False

                  
    def moveDSF(self, detectedMap):
        if self.x > 0 and detectedMap.surface[self.x - 1][self.y] == 0 and self.visitedNodes[self.x - 1][self.y] == 0:
            self.Stack.append((self.x, self.y))
            self.x = self.x - 1
            self.visitedNodes[self.x][self.y] = 1

        elif self.x < 19 and detectedMap.surface[self.x + 1][self.y] == 0 and self.visitedNodes[self.x + 1][
            self.y] == 0:
            self.Stack.append((self.x, self.y))
            self.x = self.x + 1
            self.visitedNodes[self.x][self.y] = 1

        elif self.y > 0 and detectedMap.surface[self.x][self.y - 1] == 0 and self.visitedNodes[self.x][self.y - 1] == 0:
            self.Stack.append((self.x, self.y))
            self.y = self.y - 1
            self.visitedNodes[self.x][self.y] = 1

        elif self.y < 19 and detectedMap.surface[self.x][self.y + 1] == 0 and self.visitedNodes[self.x][
            self.y + 1] == 0:
            self.Stack.append((self.x, self.y))
            self.y = self.y + 1
            self.visitedNodes[self.x][self.y] = 1
